...
create index if not exists cases_priority_reason_idx
  on public.cases (organization_id, priority_reason);

create index if not exists cases_attention_reason_idx
  on public.cases (organization_id, attention_reason);
...
begin
  ...
  derived as (
    select
      ...
      case
        when metrics.has_critical_event then 'evento_critico'
        when metrics.has_upcoming_event then 'evento_proximo'
        when metrics.has_source_issue then 'error_fuente'
        when metrics.requires_manual_review then 'requiere_revision'
        when raise_if_unassigned_with_change and metrics.has_fresh_event and metrics.current_responsible_membership_id is null then 'sin_responsable_con_novedad'
        when metrics.has_fresh_event then 'novedad_reciente'
        when metrics.has_not_found then 'proceso_no_encontrado'
        else 'estable'
      end as next_priority_reason,
      case
  ...
  finalized as (
    select
      ...
      case
        ...
        ) = 'critical' then 'interrupcion_por_criticidad'
        when derived.has_source_issue or derived.requires_manual_review then 'elevada_por_riesgo'
        when derived.has_upcoming_event or derived.has_fresh_event then 'visible_por_novedad'
        else 'silencio_por_estabilidad'
      end as next_attention_reason,
  ...
  updated as (
    ...
    set
      ...
      priority_reason = finalized.next_priority_reason,
      priority_final = finalized.next_priority_final,
      attention_level = finalized.next_attention_level,
      attention_reason = finalized.next_attention_reason,
